	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
			...
			*/
			...
			// If item has_children and item is top menu, add attributes to <a>
			// Also, add attributes to <a> if .has-megu-menu is present.

			if ( $args->has_children && $depth === 0 ) {
				$atts['href']        = '#';
				$atts['data-toggle'] = 'dropdown';
				$atts['class']       = 'nav-link dropdown-toggle';
			} elseif ( in_array( 'has-mega-menu', $classes ) ) {
				$atts['href']        = '#';
				$atts['data-toggle'] = 'dropdown';
				$atts['class']       = 'nav-link dropdown-toggle';
			} else {
			...
			// Call custom sidebar for mega menu items
			...
			// Original nav walker produces an extra HTML element for the caret.
			// ASU Web Standards produces this effect with ::after instead. No additional element needed.
			// Original code for the caret follows:
			// $item_output .= ( $args->has_children && 0 === $depth ) ? ' <span class="caret"></span></a>' : '</a>';

			$item_output .= '</a>';

			// Finally, if has-mega-menu present, call the dynamic sidebar.
			// Test to see if there's anything in it first.
			if ( in_array( 'has-mega-menu', $classes ) ) {
				if ( is_active_sidebar( 'megamenu-widget-area-' . $item->ID ) ) {
                    $item_output .= '<ul class="dropdown-menu mega-menu" role="menu">';

                    // output buffering
                    // https://wordpress.stackexchange.com/questions/22943/how-do-i-get-sidebar-into-a-varaible
                    ob_start();
					dynamic_sidebar( 'megamenu-widget-area-' . $item->ID );
					$sidebar = ob_get_contents();
					ob_end_clean();
                    $item_output .= $sidebar . '</ul>';
                }
            }
